...
> = {
  UPDATE_ROLE:      { label: 'Rolle geändert',      emoji: '👤', color: 'text-blue-300',   bg: 'bg-blue-500/10 border-blue-500/20' },
  DELETE_USER:      { label: 'User gelöscht',        emoji: '🗑️', color: 'text-red-300',    bg: 'bg-red-500/10 border-red-500/20' },
  DELETE_WITZ:      { label: 'Witz gelöscht',        emoji: '📝', color: 'text-red-300',    bg: 'bg-red-500/10 border-red-500/20' },
  RESOLVE_REPORT:   { label: 'Meldung ignoriert',    emoji: '✓',  color: 'text-green-300',  bg: 'bg-green-500/10 border-green-500/20' },
  BAN_USER:         { label: 'User gebannt',         emoji: '🔨', color: 'text-orange-300', bg: 'bg-orange-500/10 border-orange-500/20' },
  UNBAN_USER:       { label: 'User entbannt',        emoji: '🔓', color: 'text-green-300',  bg: 'bg-green-500/10 border-green-500/20' },
  WARN_USER:        { label: 'User verwarnt',        emoji: '⚠️', color: 'text-yellow-300', bg: 'bg-yellow-500/10 border-yellow-500/20' },
  DELETE_WARNING:   { label: 'Verwarnung gelöscht',  emoji: '🗑️', color: 'text-gray-300',   bg: 'bg-gray-500/10 border-gray-500/20' },
};
            ...
            {!loading && filtered.length === 0 && (
                <div className="text-center py-16">
                  ...
                  <p className="text-gray-500 text-sm">
                    {search ? 'Keine Treffer für deine Suche' : 'Keine Einträge gefunden'}
                  </p>
                  {search && (
                      <button
                          onClick={() => setSearch('')}
                ...
                <div className="divide-y divide-gray-800/50">
                  {Object.entries(grouped).map(([date, dateLogs]) => (
                    ...
                    <span className="text-gray-500 text-xs font-semibold uppercase tracking-wider">
                      ...
                      {dateLogs.length} {dateLogs.length === 1 ? 'Eintrag' : 'Einträge'}
                    </span>
                        </div>
                                ...
                                {new Date(log.createdAt).toLocaleTimeString('de-DE', {
                                  hour: '2-digit', minute: '2-digit',
                                    ...
                                    <p className="text-gray-600 text-xs mt-1">
                                      von{' '}
                                      <span
                                          onClick={() => router.push(`/profil/${log.admin.username}`)}
