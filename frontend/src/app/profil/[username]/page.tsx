...
const ROLE_CONFIG: Record<string, { label: string; emoji: string; color: string }> = {
  ADMIN: { label: 'Admin', emoji: '🛡️', color: 'bg-red-500/20 text-red-300 border-red-500/30' },
  BETA: { label: 'Beta-User', emoji: '🧪', color: 'bg-purple-500/20 text-purple-300 border-purple-500/30' },
  MODERATOR: { label: 'Moderator', emoji: '⚖️', color: 'bg-blue-500/20 text-blue-300 border-blue-500/30' },
  USER: { label: 'User', emoji: '👤', color: 'bg-gray-500/20 text-gray-300 border-gray-500/30' },
};
...
export default function PublicProfilePage() {
  ...
  const [badges, setBadges] = useState<{ emoji: string; label: string; description: string }[]>([]);
    ...
    return (
        <AppLayout>
          <div className="text-center py-20">
            ...
            <h1 className="text-2xl font-bold text-white mb-2">User nicht gefunden</h1>
          ...
          <div className="bg-gray-900/80 backdrop-blur-xl border border-gray-800/50 rounded-3xl overflow-hidden">
            ...
            <div className="px-6 pb-6">
              ...
              <p className="text-gray-600 text-xs mt-2">
                Dabei seit {new Date(profile.memberSince).toLocaleDateString('de-DE', { month: 'long', year: 'numeric' })}
              </p>
            </div>
            ...
            <div className="grid grid-cols-3 border-t border-gray-800/50">
              {[
                ...
                { label: 'Streak', value: `🔥 ${profile.currentStreak}` },
                  ...
                  <div key={i} className={`py-4 text-center ${i < 2 ? 'border-r border-gray-800/50' : ''}`}>
            ...
            {profile.witze.length === 0 ? (
                ...
                <div className="space-y-3">
                  {profile.witze.map((w) => (
                      ...
                      >
                        <p className="text-gray-100 leading-relaxed mb-2">"{w.text}"</p>
                      ...
                      {new Date(w.createdAt).toLocaleDateString('de-DE', {
                        day: 'numeric', month: 'short', year: 'numeric',
                          ...
                          <span className="ml-auto text-indigo-400 font-bold">❤️ {w.likes}</span>
                        </div>
                      </div>
                  ))}
                </div>
