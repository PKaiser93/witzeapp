...
export default function FollowingPage() {
  ...
  }, [isLoggedIn, accessToken, refreshToken, router]);

  useEffect(() => {
    // Wenn kein User: direkt zum Login
    if (!isLoggedIn) {
      router.push('/login');
      return;
    }
    ...
    loadFeed();
  }, [isLoggedIn, accessToken, loadFeed, router]);

  const handleShare = (e: React.MouseEvent, id: number) => {
    e.stopPropagation();
    navigator.clipboard
      .writeText(`${window.location.origin}/witze/${id}`)
      .then(() => {
        setCopiedId(id);
        setTimeout(() => setCopiedId(null), 2000);
      })
      .catch(() => {
        // optional: Toast / Fehlermeldung
      });
  };
