...
export default function LeaderboardPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      // eigene User-ID für Highlight laden (nur, wenn eingeloggt)
                ...
                {podiumOrder.map((entry, i) => {
                  ...
                  return (
                    <div
                      ...
                      className={`flex-1 cursor-pointer bg-gradient-to-b ${cfg.gradient} border ${cfg.border} rounded-2xl p-4 text-center transition-all hover:scale-105 ${
                        isMe ? 'ring-2 ring-indigo-500/50' : ''
                      }`}
                    >
                      <div className="text-3xl mb-2">{cfg.medal}</div>
                      <div
                        className={`w-12 h-12 mx-auto rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-black text-lg mb-2 ${
                          pos === 0 ? 'w-14 h-14 text-xl' : ''
                        }`}
                      >
                      ...
                      <div className="flex items-center justify-center gap-1">
                        <p
                          className={`font-black text-sm truncate ${cfg.text}`}
                        >
                          @{entry.username}
                        </p>
                      ...
                      <div
                        className={`${cfg.height} flex flex-col items-center justify-end`}
                      >
                  ...
                  {rest.map((entry, i) => {
                    ...
                    return (
                      <div
                        key={entry.id}
                        onClick={() => router.push(`/profil/${entry.username}`)}
                        className={`flex items-center gap-4 px-6 py-4 cursor-pointer hover:bg-gray-800/30 transition-all ${
                          isMe
            ...
            {currentUserId && !entries.find((e) => e.id === currentUserId) && (
              <div className="mt-4 p-4 bg-indigo-500/10 border border-indigo-500/20 rounded-2xl text-center">
                <p className="text-indigo-400 text-sm">
                  Du bist noch nicht in den Top 10 – poste mehr Witze und sammle
                  Likes! 🚀
                </p>
              </div>
            )}
